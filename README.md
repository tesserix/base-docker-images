...
- **Every Saturday 03:00 UTC** — `weekly-rebuild.yml` runs. Each image is
  built with `docker build --pull` so upstream alpine/debian/node index
...
- **Downstream notify** — opt-in consumer repos (FanZone, mark8ly,
  HomeChef) get a `repository_dispatch: base-image-refresh` after a
  successful weekly run, and can wire their own rebuild workflow on top.
...
4. Open a PR. CI builds, Trivy-scans, and publishes `sha-*` only. The
...
```yaml
# .github/workflows/base-refresh.yml
name: Rebuild on base image refresh
on:
  repository_dispatch:
